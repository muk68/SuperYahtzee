...
namespace DragDrop
{
    ...
    public class DragDrop
    {
        ...
        /// </summary>
        private static DragDrop Instance => _instance.Value;
        ...
        public static readonly DependencyProperty ItemDroppedProperty = DependencyProperty.RegisterAttached(
            "ItemDropped", typeof(ICommand), typeof(DragDrop), new PropertyMetadata(AttachOrRemoveItemDroppedEvent));
        ...
        private void DragSource_PreviewMouseLeftButtonDown(Object sender, MouseButtonEventArgs e)
        {
            try
            {
                ...
                _topControl = (UserControl)FindAncestor(typeof(UserControl), visual);



                _initialMousePosition = e.GetPosition(_topControl);
                ...
                _dragDropPreviewControlDataContext = GetDragDropPreviewControlDataContext(sender as DependencyObject);

                if (_dragDropPreviewControlDataContext == null)
                {
                    var frameworkElement = sender as FrameworkElement;
                    if (frameworkElement != null)
                        _dragDropPreviewControlDataContext = frameworkElement.DataContext;
                }


                _itemDroppedCommand = GetItemDropped(sender as DependencyObject);
        ...
        private void DragSource_PreviewMouseMove(Object sender, MouseEventArgs e)
        {
            ...
            if (IsMovementBigEnough(_initialMousePosition, e.GetPosition(_topControl)) == false)
            {
            ...
            var visual = e.OriginalSource as Visual;
            var control = (UserControl)FindAncestor(typeof(UserControl), visual);
            if (control != null)
            {
                _dragDropPreviewControl.Width = control.Width;
                _dragDropPreviewControl.Height = control.Height;
            }


            if (!_dragDropContainer.Children.Contains(_dragDropPreviewControl))
            {
            ...
            {
                Canvas.SetLeft(_dragDropPreviewControl, _initialMousePosition.X - _dragDropPreviewControl.Width / 2);
                Canvas.SetTop(_dragDropPreviewControl, _initialMousePosition.Y - _dragDropPreviewControl.Height / 2);
            }
        ...
        private void DragDropContainer_PreviewMouseMove(Object sender, MouseEventArgs e)
        {
            ...
            var currentPoint = e.GetPosition(_topControl);
            ...
            if (control != null)
            {
                _dragDropPreviewControl.Width = control.Width;
                _dragDropPreviewControl.Height = control.Height;
                currentPoint.X = currentPoint.X - _dragDropPreviewControl.Width / 2;
                currentPoint.Y = currentPoint.Y - _dragDropPreviewControl.Height / 2;
            }
